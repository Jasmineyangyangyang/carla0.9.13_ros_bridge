...
def main(args=None):
    ...
    while True:
        ...
        x4=float(x1+math.sin(-float(ego_yaw[i]))*0.4+math.cos(-float(ego_yaw[i]))*0.3)
        y4=float(-y1-math.cos(-float(ego_yaw[i]))*0.4+math.sin(-float(ego_yaw[i]))*0.3)
        spectator.set_transform(carla.Transform(carla.Location(x=x4,y=y4,z=1.3),carla.Rotation(0,-yaw1,0)))
        x3=float(ego_x[i-1])
        y3=float(ego_y[i-1])
