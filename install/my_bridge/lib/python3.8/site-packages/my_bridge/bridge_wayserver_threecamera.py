...
FRONT_CAMERA_Y = -0.35
FRONT_CAMERA_Z = 1.2
FRONT_CAMERA_ROLL = -5.0
FRONT_CAMERA_PITCH = 0.0
FRONT_CAMERA_YAW = 0.0
FRONT_CAMERA_IMAGE_SIZE_X = str(3840)
FRONT_CAMERA_IMAGE_SIZE_Y = str(1080)
FRONT_CAMERA_FOV = str(93)
...
class CarlaRosBridge(Node):
    ...
    def initialize_bridge(self, carla_world):
        ...
        self.window1 = tk.Tk()  # 实例化TK
        self.window1.title("Speed")
        self.window1.geometry('300x100+900+900')
        ...
        # front_camera_transform = carla.Transform(carla.Location(x=FRONT_CAMERA_X, y=FRONT_CAMERA_Y, z=FRONT_CAMERA_Z),\
        #                                               carla.Rotation(roll=FRONT_CAMERA_ROLL, pitch=FRONT_CAMERA_PITCH, yaw=FRONT_CAMERA_YAW))
        # front_camera_bp = self.blueprint_library.find('sensor.camera.rgb')
        # front_camera_bp.set_attribute('image_size_x', FRONT_CAMERA_IMAGE_SIZE_X)
        # front_camera_bp.set_attribute('image_size_y', FRONT_CAMERA_IMAGE_SIZE_Y)
        # front_camera_bp.set_attribute('fov', FRONT_CAMERA_FOV)
        # front_camera = self.carla_world.spawn_actor(front_camera_bp, front_camera_transform, attach_to=self.ego_vehicle)
        # front_camera.listen(lambda image: self.front_camera_callback(image))
        # self.sensor_list.append(front_camera)
    ...
    def front_camera_callback(self, carla_image):
        """
        ...
        # direct camera show
        cv_array = carla_image_data_array[:,:,:3]
        cv2.imshow("front_view", cv_array)
        ...
        # ros_image.header.frame_id = 'frontcamera'
        # self.front_camera_publisher.publish(ros_image)
    ...
    def left_camera_callback(self, carla_image):
        ...
        # # direct camera show
        # cv_array = carla_image_data_array[:,:,:3]
        # cv2.imshow("left_view", cv_array)
        # cv2.moveWindow("left_view",250,700)
        # cv2.waitKey(1)  # update
    ...
    def right_camera_callback(self, carla_image):
        ...
        # direct camera show
        # cv_array = carla_image_data_array[:,:,:3]
        # cv2.imshow("right_view", cv_array)
        # cv2.moveWindow("right_view",2500,700)
        # cv2.waitKey(1)  # update
    ...
    def speedshow(self, str1):
        """
        现在SpeedGoat中传过来的只有Pose，如果加上速度之后就可以在
        self.ego_transform_callback中调用这个速度显示函数
        """
        Label(self.window1,text=str1,font=("宋体",20),fg = "black" , width = 12,height = 2).place(x = 50,y = 20)
        self.window1.update()
        self.window1.after(2)       
