...
class CarlaRosBridge(Node):
    ...
    def initialize_bridge(self, carla_world):
        ...
        self.adjacent_transform_subscription = self.create_subscription(Pose, "/carla/adjacent_vehicle/control/set_transform", \
                                                                   self.adjacent_transform_callback, 10)
    ...
    def adjacent_transform_callback(self, pose):
        ...
        euler = quat2euler(quaternion)  # rad: roll, pitch, yaw
        adjacent_transform = carla.Transform(carla.Location(x, y, z), \
                                        carla.Rotation(pitch=math.degrees(euler[1]), \
                                                    yaw=math.degrees(-1*euler[2]), roll=math.degrees(euler[0])))
