...
class CanNode(Node):
    ...
    def printframe(self, db, frame, data):
        ...
        elif msg.name == "VehicleVx":
            for bsig in bmsg:
                if bsig.name == "vx":
                    data["vx"] = float(bsig.value)
        # print('┏', msg.name)
    ...
    def read_can_data(self, db):
        try:
            data = {"Xb": None, "Yb": None, "Yaw": None, "vx": None} 
            print("Listening...")
            # 解析 CAN 帧数据
            while True:
            ...
            yaw = data["Yaw"]
            vx = data["vx"]
            self.get_logger().info(f"x = {x} m, y={y} m, yaw={yaw} rad, vx={vx} km/h")
            # 构造 ROS2 消息
            ...
            adjacent_vehicle_transform = Pose()
            frex1=float(FreX[-1])+vx/3.6*(1/60)
            Frenet2Cartesian(frex1)
            FreX.append(frex1)
            adjacent_x=float(Tra_X[-1])
            adjacent_y=float(Tra_Y[-1])
            adjacent_yaw=Cal_yaw()
            adjacent_vehicle_transform.position.x = adjacent_x
            adjacent_vehicle_transform.position.y = adjacent_y
            adjacent_vehicle_transform.position.z = 0.0
            quat = euler2quat(0.0, 0.0, adjacent_yaw)  #将角度转成弧度
            adjacent_vehicle_transform.orientation.w = quat[0]
            adjacent_vehicle_transform.orientation.x = quat[1]
            adjacent_vehicle_transform.orientation.y = quat[2]
            adjacent_vehicle_transform.orientation.z = quat[3]
...
def parse_args(argv):
    ...
    parser.add_argument(
        '--receiveDB', default="/home/cec20/Yangjiaxin/my-carla-ros-bridge/src/kvaser_parser_ros/kvaser_parser_ros/dbc/vehicle_example_vx.dbc", \
