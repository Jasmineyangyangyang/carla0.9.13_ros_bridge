...
class CanNode(Node):
    def __init__(self, name, channel, bitrate, receivedb):
        ...
        # 创建单独的线程读取CAN消息
        self.datadict_queue = queue.Queue(maxsize=20000)  #speedgoat发送频率50Hz
        self.datadict_queue_lock = threading.Lock()
        self.read_thread = threading.Thread(target=self.read_can_data, args=(self.receive_dbc,))   #args=()这里只有一个元素时一定要有逗号
        ...
        self.timer = self.create_timer(1.0/60.0, self.publish_message)
    ...
    def read_can_data(self, db):
        try:
            data = {"Xb": None, "Xb_time":None ,"Yb": None, "Yb_time":None, "Yaw": None, "Yaw_time":None}
            print("Listening...")
            # 解析 CAN 帧数据
            while rclpy.ok():
                ...
                if frame.id in send_id_list:
                    ...
                    if all(data.values()):   #检查所有键是否都有值
                        with self.datadict_queue_lock:
                            self.datadict_queue.put(data)
                        # print(data)
    ...
    def publish_message(self):
        try:
            with self.datadict_queue_lock:
                data = self.datadict_queue.get_nowait()  # get是阻塞方法，如果队列空他会一直等待，get_nowat是非阻塞方法，队列为空他会引发Empty异常，监控频率了相当于
                x = data["Xb"]
                y = data["Yb"]
                yaw = data["Yaw"]
                time = data["Xb_time"]
                self.get_logger().info(f"time = {time}s, x = {x} m, y={y} m, yaw={yaw} rad")  # 这是从底层传过来的和carla世界一样的坐标
                ...
                roll = 0.0  # deg
                pitch = 0.0 # deg
                yaw = yaw    # 要换成ROS2坐标系的坐标-y, -yaw，bridge在传回carla世界时会自己转换
                ...
                # self.set_driver_view(ego_vehicle_transform)
    ...
    def __del__(self):
        self.ch.busOff()
        self.ch.close()
        self.read_thread.join()   #确保线程结束后再关闭节点
